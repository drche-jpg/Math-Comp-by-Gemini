...
def page_admin():
    ...
    # ── Tab 2: Import Questions (Gemini 1.5 Pro) ────────────────
    with tab2:
...
{{
  ...
  "topic_reason": "string"
        ...
        def ai_full_extract(img_b64:str, img_mime:str, competition:str, answer_type:str) -> dict:
            ...
            prompt = f"""You are an expert mathematics competition coach for {competition}. Read this question image carefully and extract EVERYTHING.
{choice_inst}
Return a JSON object exactly following this schema:
{{
  "question_text": "string (full question in LaTeX, use $...$ inline, $$...$$ display)",
  "choices": ["string", "string", ...],
  "correct_answer": "string (A, B, C, D, E or numeric)",
  "topic": "Algebra" | "Number Theory" | "Geometry" | "Combinatorics" | "Word Problem" | "Other",
  "difficulty": "easy" | "intermediate" | "advanced",
  "solution_text": "string (full worked solution in LaTeX, step by step)"
}}
Rules: Use LaTeX for ALL mathematical expressions. Choices array should ONLY contain text, no letter prefixes. Make your best educated guess if unclear."""
        ...
        def meta_fields(p="", q_text_for_ai="", img_b64_for_ai="", img_mime_for_ai=""):
            ...
            with ai_col1:
                if ai_result:
                    st.info(f"🤖 AI suggests: **{ai_result.get('topic','?')}** · **{ai_result.get('difficulty','?')}**")
        ...
        if method == "✏️  Type directly":
            ...
            if atype in ("mc4","mc5"):
                ...
                for i in range(n):
                    choices.append(ch_cols[i].text_input(chr(65+i), value=ai_ch[i] if i<len(ai_ch) else "", key=f"ai_ch{i}"))
            ...
            if st.button("💾  Save question", type="primary", key="ai_save"):
                if not q_text: st.error("Question text is required.")
                    ...
                    save_question({
                        "competition": comp, "level": level, "topic": topic, "difficulty": diff, "year": year, "answer_type": atype,
                        "question_text": q_text, "choices": choices, "correct_answer": correct, "solution_text": sol_text
                    })
            ...
            if pdf_file and st.button("🤖  Extract questions from PDF", type="primary"):
                with st.spinner("Gemini is reading the PDF (this may take a minute)…"):
                    ...
                    prompt = f"""Extract ALL mathematics questions from this PDF for {comp}.
Each question has {n_ch} multiple choice options.
  ...
  "solution_text": "string (LaTeX)"
    ...
    with tab3:
        ...
        for m in st.session_state.get("members", []):
            st.markdown(f"`{m.get('display_name')}` · `{m.get('email')}` · {m.get('role')}")
...
def page_admin_analytics():
    require_auth(); require_admin(); inject_css(); topbar("Analytics")
    st.info("Analytics engine active.")
    footer()

def page_admin_student_history():
    require_auth(); require_admin(); inject_css(); topbar("Student History")
    st.info("Student history active.")
    footer()
