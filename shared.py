...
Student-facing pages: login, dashboard, exam, result, history, leaderboard
...
def page_dashboard():
    ...
    sessions = []
    try:
        sessions = [s.to_dict() for s in db.collection("users").document(uid).collection("exam_sessions").order_by("timestamp_start",direction=firestore.Query.DESCENDING).limit(5).stream()]
    except: pass
